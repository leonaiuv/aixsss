...
## 更新摘要
<<<<<<< HEAD:.qoder/repowiki/zh/content/核心功能详解/角色管理/角色管理.md
**已做更改**
- 更新了角色管理组件和角色状态存储的文档，以反映新增的`primaryColor`和`secondaryColor`属性
- 在“详细组件分析”部分添加了对角色色彩配置功能的描述
- 更新了类图以包含新的色彩属性
- 添加了对色彩配置在AI生成中的应用说明
=======
**变更内容**  
...
>>>>>>> 4ed8f1c78b0b43806d00d5d2f4d821747026b1c5:.qoder/repowiki/zh/content/核心功能详解/角色管理.md
...
## 简介
<<<<<<< HEAD:.qoder/repowiki/zh/content/核心功能详解/角色管理/角色管理.md
本章节介绍“角色管理”模块的设计目标与能力范围。该模块围绕角色的生命周期进行管理，包括角色创建、编辑、删除；基于简短描述的一键生成完整角色卡（外观/性格/背景）；以及为角色生成定妆照提示词（支持 Midjourney、Stable Diffusion、通用格式）。同时，角色管理与项目画风配置联动，确保角色视觉风格的一致性，并通过本地存储实现跨会话持久化。新增了角色主色和辅色配置功能，支持角色外观的色彩设定，这些色彩信息将传递给AI生成过程，确保角色视觉特征的一致性。
=======
...
- 角色管理组件（CharacterManager）
  - 提供两步式表单：基础信息（名称/外观/性格/背景/主色/辅色）与定妆照提示词（MJ/SD/通用）
  - 一键生成角色卡与定妆照提示词，自动注入项目画风和角色色彩
  - 角色列表展示、编辑、删除
  - 提示词复制到剪贴板
...
## 架构总览
角色管理的运行时交互流程如下：
- 用户在角色管理组件中填写简短描述，点击“一键生成”，组件通过工厂创建AI客户端，拼接包含项目画风和角色色彩的提示词，调用AI生成角色卡并解析JSON填充表单
...
### 角色管理组件（CharacterManager）
- 两步式流程
  - 基础信息步骤：简短描述输入、一键生成角色卡、名称/外观/性格/背景/主色/辅色
  - 定妆照步骤：展示角色信息概览、生成/重新生成定妆照提示词、复制到剪贴板
- 画风与色彩传递
  - 读取当前项目画风配置（新/旧两种形态），在生成角色卡与定妆照提示词时注入到提示词模板
  - 新增角色主色和辅色配置，这些色彩信息将传递给AI生成过程，确保角色视觉特征的一致性
- AI生成逻辑
  - 通过工厂创建客户端，构造包含项目上下文和角色色彩的提示词，调用chat接口，解析JSON响应并填充表单
- 交互与校验
...
- 角色实体
<<<<<<< HEAD:.qoder/repowiki/zh/content/核心功能详解/角色管理/角色管理.md
  - 字段覆盖：名称、简短描述、外观、性格、背景、主色、辅色、关系、出场记录、定妆照提示词、自定义画风等
  - 新增`primaryColor`和`secondaryColor`字段用于角色色彩配置，同时保留`themeColor`作为向后兼容
=======
...
- 角色引用
  - 在分镜细化中可引用角色信息（名称、外观、性格、主色、辅色）到场景描述中，便于AI生成更一致的视觉描述
- 出场记录
...
## 结论
<<<<<<< HEAD:.qoder/repowiki/zh/content/核心功能详解/角色管理/角色管理.md
角色管理模块通过两步式表单、AI生成与本地持久化，提供了从角色设定到定妆照提示词的完整工作流。其与项目画风配置深度耦合，确保角色视觉风格的一致性；与分镜细化协作，进一步提升生成内容的连贯性。新增的角色主色和辅色配置功能，支持角色外观的色彩设定，这些色彩信息将传递给AI生成过程，确保角色视觉特征的一致性。类型系统与工厂模式使扩展新的AI供应商与数据结构变更变得简单可靠。
=======
