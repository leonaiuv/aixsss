...
## 更新摘要
**变更内容**
<<<<<<< HEAD:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化/分镜细化.md
- 将分镜细化流程从三阶段扩展为四阶段，新增“对话生成”阶段
- 更新架构总览、详细组件分析、依赖分析等章节以反映新增的对话生成流程
- 更新序列图和流程图以包含第四阶段
- 更新单元测试覆盖策略以涵盖对话生成阶段
=======
- 在分镜细化工作流中新增第四阶段“台词生成”，支持生成对白、独白、旁白和心理活动四种类型台词
- 更新“一键生成全部”流程，将台词生成作为第四阶段集成
- 新增台词解析逻辑，将AI生成的文本结构化为对话行数据
- 更新用户界面，增加台词展示区域与复制功能
- 增强单元测试覆盖，验证四阶段完整流程
>>>>>>> 4ed8f1c78b0b43806d00d5d2f4d821747026b1c5:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化.md
...
## 简介
<<<<<<< HEAD:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化/分镜细化.md
本文件围绕“分镜细化”模块进行深入技术剖析，重点说明其对初始分镜进行内容增强与结构优化的技术路径。该模块通过四阶段逐步细化：场景描述、动作描述、镜头提示词和对话生成，借助AI服务在不同维度提升画面描述质量、对白与镜头语言表达力。文档同时对比“批量处理”与“逐条编辑”的实现差异及状态同步机制，结合单元测试覆盖策略验证关键逻辑的稳定性，并给出用户反馈（进度条、完成提示）与高并发请求下的错误恢复与数据一致性保障建议。
=======
本文件围绕“分镜细化”模块进行深入技术剖析，重点说明其对初始分镜进行内容增强与结构优化的技术路径。该模块通过四阶段逐步细化：场景描述、动作描述、镜头提示词、**台词生成**，借助AI服务在不同维度提升画面描述质量、对白与镜头语言表达力。文档同时对比“批量处理”与“逐条编辑”的实现差异及状态同步机制，结合单元测试覆盖策略验证关键逻辑的稳定性，并给出用户反馈（进度条、完成提示）与高并发请求下的错误恢复与数据一致性保障建议。
>>>>>>> 4ed8f1c78b0b43806d00d5d2f4d821747026b1c5:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化.md
...
## 核心组件
- 分镜细化组件：负责四阶段生成、逐条编辑、批量处理、导航与状态同步；使用进度条与错误提示反馈用户。
...
## 架构总览
分镜细化采用“渐进式四阶段+批量一键生成”的双模式设计：
- 逐条编辑模式：按阶段顺序生成，每步可手动编辑，状态写回本地存储并驱动UI更新。
- 批量处理模式：在当前分镜缺失关键字段时，依次执行四阶段生成，期间通过状态读取确保前后一致性。
...
C->>P : 调用chat(对话生成)
P-->>C : 返回内容
C->>S : updateScene(写入对话/状态)
=======
C->>P : 调用chat(台词生成)
P-->>C : 返回内容
C->>S : updateScene(写入台词/状态)
>>>>>>> 4ed8f1c78b0b43806d00d5d2f4d821747026b1c5:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化.md
S->>L : 保存分镜
C-->>U : 展示完成/进度
```
...
- 对话生成：基于分镜概要、场景描述和角色信息，生成符合场景氛围的对白、独白、旁白和心理活动，写入对话字段并更新状态。
=======
- **场景描述生成**：基于项目风格、主角、当前分镜概要与前一分镜概要，生成环境、氛围、光影等描述，写入场景描述字段并更新状态。
- **动作描述生成**：基于场景描述与分镜概要，生成角色动作、表情、肢体语言，写入动作描述字段并更新状态。
- **镜头提示词生成**：整合场景与动作描述，生成面向AI绘图的提示词，写入镜头提示词字段并更新状态；若为最后一个分镜，更新项目工作流状态。
- **台词生成**：基于分镜概要、场景描述和角色信息，生成对白、独白、旁白和心理活动四种类型台词，写入`dialogues`字段并更新状态为`completed`。
>>>>>>> 4ed8f1c78b0b43806d00d5d2f4d821747026b1c5:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化.md
...
- 批量处理：一键生成全部，内部串行四阶段，期间通过状态读取确保每步完成后才进入下一步；防重复触发，失败时展示错误信息。
...
Stage3 --> Check3{"是否成功?"}
Check3 --> |否| Error3["抛错并显示错误"] --> End
<<<<<<< HEAD:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化/分镜细化.md
Check3 --> |是| Stage4["生成对话"]
=======
Check3 --> |是| Stage4["生成台词"]
>>>>>>> 4ed8f1c78b0b43806d00d5d2f4d821747026b1c5:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化.md
Stage4 --> Done["标记完成/更新项目状态"]
Done --> End
```
...
- 一键生成流程中，使用store.getState读取最新场景数据，确保四阶段之间的一致性。
...
- **新增台词反馈**：支持展示已生成的台词列表，并提供单条复制与全部复制功能。
...
### 单元测试覆盖策略
<<<<<<< HEAD:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化/分镜细化.md
- 一键生成全部流程：模拟四次API调用，分别更新场景描述、动作描述、镜头提示词和对话，断言updateScene调用次数与参数。
=======
- 一键生成全部流程：模拟四次API调用，分别更新场景描述、动作描述、镜头提示词、**台词**，断言updateScene调用次数与参数。
>>>>>>> 4ed8f1c78b0b43806d00d5d2f4d821747026b1c5:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化.md
...
## 结论
<<<<<<< HEAD:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化/分镜细化.md
分镜细化模块通过四阶段精细化提示词构建，实现了从“分镜概要”到“场景描述”“动作描述”再到“镜头提示词”和“对话生成”的结构化升级。逐条编辑满足局部优化需求，批量处理则提供一键式高效产出。状态同步通过store与本地存储保障一致性，错误处理与防重复触发增强了稳定性。单元测试覆盖关键流程与边界情况，为功能可靠性提供保障。
=======
分镜细化模块通过四阶段精细化提示词构建，实现了从“分镜概要”到“场景描述”“动作描述”再到“镜头提示词”和“**台词生成**”的结构化升级。逐条编辑满足局部优化需求，批量处理则提供一键式高效产出。状态同步通过store与本地存储保障一致性，错误处理与防重复触发增强了稳定性。单元测试覆盖关键流程与边界情况，为功能可靠性提供保障。
>>>>>>> 4ed8f1c78b0b43806d00d5d2f4d821747026b1c5:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化.md
...
<<<<<<< HEAD:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化/分镜细化.md
  - [对话生成](file://manga-creator/src/components/editor/SceneRefinement.tsx#L381-L470)
=======
  - [台词生成](file://manga-creator/src/components/editor/SceneRefinement.tsx#L381-L470)
>>>>>>> 4ed8f1c78b0b43806d00d5d2f4d821747026b1c5:.qoder/repowiki/zh/content/技术架构/组件架构/核心组件/编辑器工作流/分镜细化.md
- 一键生成全部
