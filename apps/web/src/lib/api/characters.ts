...
export async function apiListCharacters(projectId: string) {
  return apiRequest<ApiCharacter[]>(`/projects/${encodeURIComponent(projectId)}/characters`, {
    method: 'GET',
  });
}
...
export async function apiCreateCharacter(
  projectId: string,
  input: Partial<ApiCharacter> & Pick<Character, 'name'>,
) {
  const body: Record<string, unknown> = {
    ...(typeof input.id === 'string' ? { id: input.id } : {}),
    name: input.name,
    ...(typeof input.briefDescription === 'string'
...
export async function apiUpdateCharacter(
  projectId: string,
  characterId: string,
  updates: Partial<ApiCharacter>,
) {
  const body: Record<string, unknown> = {
    ...(typeof updates.name === 'string' ? { name: updates.name } : {}),
    ...(typeof updates.briefDescription === 'string'
      ? { briefDescription: updates.briefDescription }
      : {}),
