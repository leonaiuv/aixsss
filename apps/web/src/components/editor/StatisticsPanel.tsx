...
function InsightBadge({ children, variant = 'default' }: { children: React.ReactNode, variant?: 'default' | 'positive' | 'negative' | 'warning' }) {
  const colors = {
  ...
  return (
    <div className={cn("inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border", colors[variant])}>
      {children}
    </div>
  );
}
...
// 自动生成数据洞察
function AIUsageInsights({ stats, _trend }: { stats: ReturnType<typeof calculateUsageStats>, _trend: { name: string; calls: number; errors: number; tokens: number }[] }) {
  const insights = useMemo(() => {
    const list: Array<{ icon: ReactNode, text: string, variant: 'default' | 'positive' | 'negative' | 'warning' }> = [];

    // Success Rate Insight
    if (stats.totalCalls > 0) {
      if (stats.successRate > 98) {
        list.push({ icon: <CheckCircle className="h-3 w-3" />, text: "服务极度稳定", variant: 'positive' });
      } else if (stats.successRate < 80) {
        list.push({ icon: <AlertTriangle className="h-3 w-3" />, text: "失败率偏高，建议检查配置", variant: 'negative' });
      }
    }

    // Latency Insight
    if (stats.p95DurationMs > 30000) {
      list.push({ icon: <Clock className="h-3 w-3" />, text: "P95 耗时较长 (>30s)", variant: 'warning' });
    ...
    if (stats.totalTokens > 100000) {
      list.push({ icon: <Database className="h-3 w-3" />, text: "Token 消耗较大", variant: 'default' });
    }

    return list;
  }, [stats]);

  if (stats.totalCalls === 0) return <div className="text-sm text-muted-foreground">暂无数据供分析</div>;
...
// 迷你趋势图
function MiniTrendChart({ data, color = "#8884d8", dataKey = "value" }: { data: { name: string; calls: number; errors: number; tokens: number }[], color?: string, dataKey?: string }) {
...
// 环形 KPI
function RadialKPI({ value, label, subLabel, color = "#22c55e" }: { value: number, max?: number, label: string, subLabel?: string, color?: string }) {
  ...
  return (
    <div className="relative h-full flex items-center justify-between p-4 bg-card border rounded-xl overflow-hidden">
      <div className="z-10">
        ...
        {subLabel && <div className="text-xs text-muted-foreground mt-1 opacity-80">{subLabel}</div>}
      </div>
...
export function StatisticsPanel({ projectId }: StatisticsPanelProps) {
  ...
  const aiTrendData = useMemo(() => {
    ...
    const granularity: TrendGranularity = aiRange === '24h' ? 'hour' : aiRange === 'all' ? 'month' : 'day';
    const now = Date.now();
    const rangeStart = typeof aiFrom === 'number' ? aiFrom : Math.min(...aiTabBaseEvents.map((e) => e.completedAt));
    ...
    for (let cursor = startBucketDate; cursor.getTime() <= endBucketDate.getTime(); cursor = nextBucket(cursor, granularity)) {
      const key = cursor.getTime();
      const label = granularity === 'hour'
          ? format(cursor, 'HH:mm', { locale: zhCN }) // 简化X轴
          : granularity === 'day' ? format(cursor, 'MM-dd', { locale: zhCN }) : format(cursor, 'yyyy-MM', { locale: zhCN });
  ...
  // Actions
  const handleCopyError = useCallback(async (event: AIUsageEvent) => {
    try {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground mt-0.5">
              <span>{projectId ? (targetProject ? targetProject.title : '项目统计') : '全局概览'}</span>
        ...
        <div className="flex items-center gap-2 bg-muted/50 p-1 rounded-lg">
           <Tabs defaultValue={apiMode ? 'ai' : 'overview'} className="w-auto">
            <TabsList className="h-8 bg-transparent">
              {!apiMode && <TabsTrigger value="overview" className="h-7 text-xs px-3">总览</TabsTrigger>}
              <TabsTrigger value="ai" className="h-7 text-xs px-3">AI 监控</TabsTrigger>
              {!apiMode && <TabsTrigger value="storage" className="h-7 text-xs px-3">存储</TabsTrigger>}
            </TabsList>
           
            <TabsContent value="overview" className="hidden" /> {/* Placeholder to control active tab state via parent Tabs */}
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card className="col-span-1 lg:col-span-2 bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/30 dark:to-teal-950/30 border-emerald-100 dark:border-emerald-900">
                    ...
                    <CardContent className="flex items-center justify-between">
                        <div>
                           <div className="text-4xl font-bold text-emerald-900 dark:text-emerald-50">{completionRate.toFixed(0)}<span className="text-xl">%</span></div>
                ...
                <Card className="col-span-1 flex flex-col justify-between">
                    <CardHeader className="pb-2">
                         <CardTitle className="text-sm text-muted-foreground font-medium">项目概况</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="col-span-1 flex flex-col justify-between">
                    <CardHeader className="pb-2">
                         <CardTitle className="text-sm text-muted-foreground font-medium">资源消耗</CardTitle>
                    </CardHeader>
                    <CardContent>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 bg-muted/20 p-2 rounded-xl border">
               ...
               <div className="flex items-center gap-3 pr-2">
                  ...
                  <div className="flex gap-1">
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={handleDownloadAIUsage} title="导出数据">
                          <Download className="h-3.5 w-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={handleClearAIUsage} title="清空统计">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="col-span-1">
                   <RadialKPI 
                      ...
                      label="成功率" 
                      subLabel={aiTabStats.errorCount > 0 ? `${aiTabStats.errorCount} 次失败` : '完美运行'}
                      color={aiTabStats.successRate > 95 ? "#22c55e" : aiTabStats.successRate > 80 ? "#f59e0b" : "#ef4444"}
                   />
                </div>
                ...
                <Card className="col-span-1 shadow-sm">
                    <CardContent className="p-4 flex flex-col justify-between h-full">
                        <div>
                            ...
                            <div className="text-2xl font-bold">{formatDuration(aiTabStats.avgDurationMs)}</div>
                        </div>
                        <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground bg-muted/50 p-2 rounded">
                            <span>P95</span>
                            <span className="font-mono text-foreground">{formatDuration(aiTabStats.p95DurationMs)}</span>
                        </div>
                    </CardContent>
                </Card>
                 ...
                 <Card className="col-span-1 shadow-sm bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border-amber-100 dark:border-amber-900/50">
                    <CardContent className="p-4">
                        ...
                        <div className="text-2xl font-bold text-amber-900 dark:text-amber-100">${aiTabCostEstimateUSD.toFixed(4)}</div>
                ...
                <Card className="col-span-1 md:col-span-2 lg:col-span-3 shadow-sm flex flex-col h-full">
                        ...
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-base font-semibold flex items-center gap-2">
                             ...
                             <div className="flex gap-2">
                                <Select value={aiCallTypeFilter} onValueChange={(v) => setAiCallTypeFilter(v as 'all' | AIUsageEvent['callType'])}>
                                    ...
                                    <SelectContent>
                                        ...
                                        {aiTabAvailableCallTypes.map(t => (
                                            <SelectItem key={t} value={t}>{getCallTypeLabel(t)}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                             </div>
                    ...
                    <CardContent className="flex-1">
                        <div className="h-[350px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <ComposedChart data={aiTrendData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                    <defs>
                                    ...
                                    />
                                    <RechartsTooltip 
                                        contentStyle={{ backgroundColor: 'rgba(var(--background-rgb), 0.9)', borderRadius: '12px', border: '1px solid var(--border)', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', backdropFilter: 'blur(8px)', padding: '12px' }}
                                        itemStyle={{ fontSize: '12px', padding: '2px 0' }}
                                        labelStyle={{ color: 'var(--foreground)', fontWeight: 'bold', marginBottom: '8px', fontSize: '14px' }}
                ...
                <Card className="col-span-1 shadow-sm flex flex-col h-full">
                     ...
                     <CardContent className="flex-1 min-h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={aiCallsByType.slice(0, 8)} layout="vertical" margin={{ left: 10 }}>
                         ...
                         <ScrollArea className="h-[200px] px-4">
                             <div className="space-y-2 pb-4">
                                {aiRecentErrors.length === 0 ? (
                                    <div className="text-xs text-muted-foreground py-4 text-center">暂无错误记录</div>
                                ) : (
                                    aiRecentErrors.map((e, i) => (
                                        <div key={i} className="group flex gap-3 items-start text-xs border-b pb-2 pt-2 last:border-0 hover:bg-muted/30 transition-colors rounded-sm px-2">
                                            ...
                                            <div className="min-w-0 flex-1 grid grid-cols-1 md:grid-cols-4 gap-2 items-center">
                                                ...
                                                <div className="text-red-600/80 break-all col-span-2 line-clamp-1 group-hover:line-clamp-none transition-all" title={e.errorMessage}>
                                                    {e.errorMessage}
                                                </div>
                                            </div>
                                            <Button
                     ...
                     <div className="flex items-center gap-4">
                        <div className="flex-1">
                            <div className="flex justify-between mb-2">
                                ...
                                <span className="text-sm text-muted-foreground">{getStorageUsageSnapshot().percentage.toFixed(1)}%</span>
                            </div>
