          ...
          <div className="space-y-6">
            <div className="bg-card border rounded-xl p-5 shadow-sm space-y-4">
              ...
              <p className="text-lg font-medium text-primary">
                {toText(conflictEngine.coreObjectOrEvent)}
              </p>
            </div>
              ...
              {isRecord(conflictEngine.stakesByFaction) ? (
                <div className="space-y-4">
                  {Object.entries(conflictEngine.stakesByFaction).map(([k, v]) => (
                    <div
                      key={k}
                      className="relative pl-4 border-l-2 border-primary/20 hover:border-primary/50 transition-colors"
                    >
            ...
            {firstMover ? (
              <Card className="h-full border-primary/10 shadow-sm">
                <CardHeader>
                  <SectionHeader
                    icon={Zap}
                    title="第一推动因 (First Mover)"
                    description="引发一切连锁反应的源头"
                  />
                </CardHeader>
                <CardContent className="space-y-5">
                  ...
                  <Kv
                    label="真实意图"
                    value={firstMover.hiddenIntent}
                    icon={EyeOff}
                    className="p-2 bg-yellow-500/5 border-yellow-500/20 rounded-lg"
                  />
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <section>
        <SectionHeader
          icon={Layers}
          title="信息能见度层 (Information Layers)"
          description="从上帝视角到局外人的信息差设计"
        />
              ...
              <Card key={idx} className="overflow-hidden border-l-4 border-l-blue-500/50">
                <div className="bg-muted/30 p-3 border-b flex items-center justify-between">
                  <div className="flex items-center gap-2 font-medium">
                    <div className="bg-background border px-2 py-0.5 rounded text-xs text-muted-foreground shadow-sm">
                      Layer {idx + 1}
                    </div>
      ...
      <section>
        <SectionHeader
          icon={LayoutGrid}
          title="角色矩阵 (Character Matrix)"
          description="主要角色的驱动力与弱点分析"
        />
                    ...
                    <div className="pt-2">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {asStringArray(r.assumptions).map((a, i) => (
                          <span
                            key={i}
                            className="text-xs bg-muted px-1.5 py-0.5 rounded text-muted-foreground border"
                          >
                            {a}
                          </span>
                        ))}
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <SectionHeader
          icon={PlayCircle}
          title="节拍流程 (Beat Flow)"
          description="场景化的因果链推进"
        />
        <Badge variant="outline" className="text-xs font-mono">
          {toText(beatFlow.actMode)}
        </Badge>
      </div>
            ...
            <div key={actIdx} className="relative">
              ...
              <div className="sticky top-0 z-10 flex items-center justify-center mb-6">
                <div className="bg-background border shadow-sm px-4 py-1.5 rounded-full text-sm font-semibold flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-primary"></span>第 {toText(a.act)} 幕：
                  {nonEmptyText(a.actName)}
                </div>
              </div>
              ...
              <div className="space-y-6 pl-10 md:pl-0">
                {beats.map((beat: unknown, bIdx: number) => {
                  ...
                  return (
                    <div
                      key={bIdx}
                      className="relative group md:flex md:justify-between md:gap-8 md:items-start"
                    >
                      {/* Timeline Dot */}
                      ...
                      <div className="md:w-1/2 md:text-right md:pr-8 mb-2 md:mb-0">
                        ...
                        <h4 className="font-semibold text-base">
                          {nonEmptyText(b.beatName) || `Beat ${bIdx + 1}`}
                        </h4>
                        <div className="mt-2 flex flex-wrap gap-1 md:justify-end">
                          {asStringArray(b.characters).map((c) => (
                            <Badge key={c} variant="secondary" className="text-[10px] px-1 h-5">
                              {c}
                            </Badge>
                          ))}
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-4 text-xs text-muted-foreground pt-2 border-t border-dashed">
                            <span className="flex items-center gap-1">
                              <Drama className="w-3 h-3" /> {toText(b.emotionalTone)}
                            </span>
                            <span className="flex items-center gap-1">
                              <Lightbulb className="w-3 h-3" /> {toText(b.visualHook)}
                            </span>
                          </div>
                        </div>
                      </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-base">
                      {nonEmptyText(p.lineType) ?? 'Plot Line'}
                    </CardTitle>
                ...
                <CardContent className="space-y-3 text-sm">
                  ...
                  <div className="space-y-1">
                    <div className="text-xs font-medium text-muted-foreground">
                      关键咬合点 (Key Interlocks)
                    </div>
                    <ul className="list-disc list-inside text-muted-foreground text-xs space-y-0.5">
                      {asStringArray(p.keyInterlocks).map((k, i) => (
                        <li key={i}>{k}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="text-xs bg-destructive/10 text-destructive p-2 rounded flex gap-2">
                    ...
                    <span>
                      <b>不可逆点:</b> {toText(p.pointOfNoReturn)}
                    </span>
                  </div>
                </CardContent>
              </Card>
      ...
      <section>
        <SectionHeader
          icon={CheckCircle2}
          title="自洽校验 (Consistency Checks)"
          description="叙事逻辑的自我审查"
        />
        {consistencyChecks ? (
          <div className="bg-card border rounded-xl overflow-hidden">
            <div className="grid divide-y">
              {[
                ...
                const isPass =
                  s === 'true' || s === 'yes' || s.includes('pass') || s.includes('是');
                    ...
                    <div className="flex items-center gap-2">
                      {isPass ? (
                        ...
                        <Badge variant="destructive">
                          <AlertCircle className="w-3 h-3 mr-1" /> {toText(val)}
                        </Badge>
                      )}
                    </div>
                  </div>
              ...
              {asStringArray(consistencyChecks.notes).length > 0 && (
                <div className="p-4 bg-muted/20">
                  ...
                  <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                    {asStringArray(consistencyChecks.notes).map((n, i) => (
                      <li key={i}>{n}</li>
                    ))}
                  </ul>
                </div>
          ...
          <div>
            <CardTitle className="text-xl flex items-center gap-2">
              ...
              <Badge variant="outline" className="ml-2 font-mono text-xs font-normal">
                v{toText(chain.version) || '0.1'}
              </Badge>
            </CardTitle>
            <CardDescription className="mt-1">多阶段生成的深度叙事逻辑结构</CardDescription>
          </div>
        ...
        <Tabs defaultValue="phase1" className="w-full">
          <div className="border-b px-4 bg-background">
            <TabsList className="h-auto w-full justify-start gap-4 bg-transparent p-0">
              ...
              <TabsTrigger
                value="raw"
                className="ml-auto data-[state=active]:bg-muted py-2 rounded-md border text-xs h-8"
              >
                JSON 源数据
              </TabsTrigger>
            </TabsList>
          ...
          <ScrollArea className="h-[calc(100vh-300px)] min-h-[500px]">
            <div className="p-6">
              <TabsContent value="phase1" className="m-0 mt-0">
                <Phase1View chain={chain} />
              </TabsContent>
              <TabsContent value="phase2" className="m-0 mt-0">
                <Phase2View chain={chain} />
              </TabsContent>
              <TabsContent value="phase3" className="m-0 mt-0">
                <Phase3View chain={chain} />
              </TabsContent>
              <TabsContent value="phase4" className="m-0 mt-0">
                <Phase4View chain={chain} />
              </TabsContent>
