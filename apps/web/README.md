...
> 注：本仓库已升级为 **pnpm/turbo monorepo**。当前前端位于 `apps/web`，后端位于 `apps/api`，Worker 位于 `apps/worker`。开发/部署请优先参考仓库根目录 `README.md`。
...
- **分镜细化工作流** ✅
  - 四阶段渐进生成(场景锚点→关键帧(KF0-KF8)→时空/运动→台词)
  - 上下文保持与连贯性(自动引用前一分镜)
  - 单独重新生成或一键生成全部
  - 手动编辑与优化每个阶段内容
...
- **本地模式（local）**：API Key 采用 AES 加密存储在浏览器本地（LocalStorage），适合离线/单机使用
- **服务端模式（api，默认）**：API Key 加密存储在服务端；浏览器不保存明文，也不直接调用供应商接口
- 支持自定义 BaseURL（OpenAI Compatible）
